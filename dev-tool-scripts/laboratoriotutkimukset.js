(async () => {
    ...
    if (window.kantainLabDownload?.running) throw new Error('Laboratoriohaku on jo käynnissä.');
    const now = new Date();
    ...
    const state = { running: true, data, progress: 'Aloitetaan', cancel: () => controller.abort() };
    window.kantainLabDownload = state;
    state.save = () => {
        const url = URL.createObjectURL(new Blob([JSON.stringify(data)], { type: 'application/json' }));
        const link = document.createElement('a');
        link.href = url;
        link.download = `laboratoriotutkimukset_${data.start}_${data.end}${data.complete ? '' : '_KESKENERAINEN'}.json`;
        link.click();
    ...
    try {
        ...
        try {
            ...
            for (let page = 1; page <= pages; page++) {
                ...
                for (const visit of list.palvelutapahtumat) visits.set(visit.tempSetId, { tempSetId: visit.tempSetId, tunniste: visit.tunniste, alkuPvm: visit.alkuPvm });
                progress(`Käyntien linkitys ${page}/${pages}`);
            }
        } catch (error) {
            data.warnings.push(`Pysyviä käyntilinkkejä ei voitu hakea: ${error.message}`);
        }
        const keys = new Set();
        let maxPages = 1;
        ...
        state.running = false;
        state.save();
        progress(data.complete ? `Valmis: ${data.events.length} tuloskokonaisuutta. Avaa JSON-tiedosto Kantaimen Laboratoriotulokset-kentässä.` : `Keskeneräinen JSON tallennettu. ${data.errors.at(-1)}`);
    }
})();
